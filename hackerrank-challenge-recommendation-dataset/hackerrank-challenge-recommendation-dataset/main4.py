...
def main():
	...
	for i in id:
		...
		for jj,kk in candidate.iteritems():
			if jj not in solved and jj not in tt:
				if solve != 0 and noSolve != 0:
					if difficulty[jj] >= solve or difficulty[jj] <= noSolve:
						tt.append(jj)
				elif solve == 0:
					if difficulty[jj] <= noSolve:
						tt.append(jj)
				else:
					if difficulty[jj] >= solve:
						tt.append(jj)
			if len(tt) >= 11:
				break
